...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<novaAction type="shell">${commands.setupCommand}</novaAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<novaAction type="start">${commands.startCommand}</novaAction>
`;
  }
...
<novaArtifact id="project-setup" title="Project Setup">
${commandString}
</novaArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeNovaArtifactTags(input: string) {
  // Regular expression to match boltArtifact tags and their content
  const regex = /(<novaArtifact[^>]*>)([\s\S]*?)(<\/boltArtifact>)/g;
...
export function escapeNovaAActionTags(input: string) {
  // Regular expression to match boltArtifact tags and their content
  const regex = /(<novaAction[^>]*>)([\s\S]*?)(<\/boltAction>)/g;
...
export function escapeNovaTags(input: string) {
  return escapeNovaArtifactTags(escapeNovaAActionTags(input));
}

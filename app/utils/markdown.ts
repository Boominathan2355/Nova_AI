  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__novaThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__boltArtifact__', '__novaThought__', '__boltQuickAction', '__boltSelectedElement__'],

      // ['className', '__novaThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__boltArtifact__', '__novaThought__', '__boltQuickAction'],
    ],
  },
  strip: [],
};

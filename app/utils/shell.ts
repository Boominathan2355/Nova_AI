...
export class NovaShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newNovaShellProcess(webcontainer, terminal);
  ...
  async newNovaShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newNovaShellProcess() {
  return new NovaShell();
}

import type { ActionType, NovaAction, NovaActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { NovaArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<novaArtifact';
const ARTIFACT_TAG_CLOSE = '</novaArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<novaAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</novaAction>';
const BOLT_QUICK_ACTIONS_OPEN = '<nova-quick-actions>';
const BOLT_QUICK_ACTIONS_CLOSE = '</nova-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends NovaArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: NovaArtifactData;
  currentAction: NovaActionData;
  actionId: number;
}
    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          // Find all <nova-quick-action ...>label</nova-quick-action> inside
          const quickActionRegex = /<nova-quick-action([^>]*)>([\s\S]*?)<\/bolt-quick-action>/g;
          let match;
          const buttons = [];
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as NovaAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as NovaAction,
              });

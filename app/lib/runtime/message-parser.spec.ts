...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out nova artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <nova', 'Foo bar '],
      ['Foo bar <novaa', 'Foo bar <novaa'],
      ['Foo bar <novaA', 'Foo bar '],
      ['Foo bar <novaArtifacs></novaArtifact>', 'Foo bar <novaArtifacs></novaArtifact>'],
      ['Before <oltArtfiact>foo</novaArtifact> After', 'Before <oltArtfiact>foo</novaArtifact> After'],
      ['Before <novaArtifactt>foo</novaArtifact> After', 'Before <novaArtifactt>foo</novaArtifact> After'],
    ])('should correctly parse chunks and strip out nova artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <novaArtifact title="Some title" id="artifact_1">foo bar</novaArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <novaArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</novaArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        [
          ...
          ' ',
          '>',
          'foo</novaArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <novaArtifact title="Some title" id="artifact_1">foo</novaArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <novaArtifact title="Some title" id="artifact_1"><novaAction type="shell">npm install</novaAction></novaArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <novaArtifact title="Some title" id="artifact_1"><novaAction type="shell">npm install</novaAction><novaAction type="file" filePath="index.js">some content</novaAction></novaArtifact> After',
        {
          output: 'Before  After',

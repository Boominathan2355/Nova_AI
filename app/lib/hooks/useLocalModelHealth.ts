...
export interface UseLocalModelHealthReturn {
  ...
  performHealthCheck: (provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => Promise<void>;
  isHealthy: (provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => boolean;
...
export function useLocalModelHealth(options: UseLocalModelHealthOptions = {}): UseLocalModelHealthReturn {
  ...
  const getHealthStatus = useCallback((provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {
    return localModelHealthMonitor.getHealthStatus(provider, baseUrl);
  }, []);

  // Start monitoring a provider
  const startMonitoring = useCallback(
    (provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string, interval?: number) => {
  ...
  const stopMonitoring = useCallback((provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {
  ...
  const performHealthCheck = useCallback(async (provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {
    await localModelHealthMonitor.performHealthCheck(provider, baseUrl);
  }, []);

  // Check if a provider is healthy
  const isHealthy = useCallback(
    (provider: 'LlamaCpp' | 'LMStudio' | 'OpenAILike', baseUrl: string) => {

...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyNovaActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyNovaActions(content);

      content = content.replace(/<div class=\\"__novaThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Nova failed to select files`);
  }

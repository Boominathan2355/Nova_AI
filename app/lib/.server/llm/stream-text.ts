...
function sanitizeText(text: string): string {
  let sanitized = text.replace(/<div class=\\"__novaThought__\\">.*?<\/div>/s, '');
  sanitized = sanitized.replace(/<think>.*?<\/think>/s, '');
  sanitized = sanitized.replace(/<novaAction type="file" filePath="package-lock\.json">[\s\S]*?<\/boltAction>/g, '');

...
export function simplifyNovaActions(input: string): string {
  // Using regex to match boltAction tags that have type="file"
  const regex = /(<novaAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/boltAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<novaAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</novaAction>`;
    });

  return `<novaArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</novaArtifact>`;
}

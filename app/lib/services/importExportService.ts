...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          nova_user_profile: this._safeGetItem('nova_user_profile'),
          nova_settings: this._safeGetItem('nova_settings'),
          nova_profile: this._safeGetItem('nova_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('nova_viewed_features'),
          developer_mode: this._safeGetItem('nova_developer_mode'),
        ...
        ui: {
          // Tab configuration
          nova_tab_configuration: this._safeGetItem('nova_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('nova_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('nova_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          nova_read_logs: this._safeGetItem('nova_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('nova_last_acknowledged_version'),
        },
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.nova_tab_configuration) {
        try {
          this._safeSetItem('nova_tab_configuration', data.ui.nova_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'nova_acknowledged_debug_issues',
        'nova_acknowledged_connection_issue',
        'error_logs',
        'nova_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('nova_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {

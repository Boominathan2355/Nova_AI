...
export function LockManager() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Controls */}
      <div className="flex items-center gap-1 px-2 py-1 border-b border-nova-elements-borderColor">
        {/* Search Input */}
        <div className="relative flex-1">
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-nova-elements-textTertiary i-ph:magnifying-glass text-xs pointer-events-none" />
          <input
            type="text"
            placeholder="Search..."
            className="w-full text-xs pl-6 pr-2 py-0.5 h-6 bg-nova-elements-background-depth-2 text-nova-elements-textPrimary rounded border border-nova-elements-borderColor focus:outline-none"
            value={searchTerm}
        ...
        {/* Filter Select */}
        <select
          className="text-xs px-1 py-0.5 h-6 bg-nova-elements-background-depth-2 text-nova-elements-textPrimary rounded border border-nova-elements-borderColor focus:outline-none"
          value={filter}
      ...
      <div className="flex items-center justify-between px-2 py-1 text-xs text-nova-elements-textSecondary">
        <div>
          <Checkbox
            checked={selectAllCheckedState}
            onCheckedChange={handleSelectAll}
            className="w-3 h-3 rounded border-nova-elements-borderColor mr-2"
        ...
        {selectedItems.size > 0 && (
          <button
            className="ml-auto px-2 py-0.5 rounded bg-nova-elements-button-secondary-background hover:bg-nova-elements-button-secondary-backgroundHover text-nova-elements-button-secondary-text text-xs flex items-center gap-1"
      ...
      <div className="flex-1 overflow-auto modern-scrollbar px-1 py-1">
        {filteredAndSortedItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-nova-elements-textTertiary text-xs gap-2">
            ...
            {filteredAndSortedItems.map((item) => (
              <li
                ...
                className={classNames(
                  'text-nova-elements-textTertiary flex items-center gap-2 px-2 py-1 rounded hover:bg-nova-elements-background-depth-2 transition-colors group',
                  selectedItems.has(item.path) ? 'bg-nova-elements-background-depth-2' : '',
                )}
              >
                <Checkbox
                  checked={selectedItems.has(item.path)}
                  onCheckedChange={() => handleSelectItem(item.path)}
                  className="w-3 h-3 rounded border-nova-elements-borderColor"
                  aria-labelledby={`item-label-${item.path}`} // For accessibility
                />
                <span
                  className={classNames(
                    'shrink-0 text-nova-elements-textTertiary text-xs',
                ...
                ></span>
                <button
                  className="flex items-center px-1 py-0.5 text-xs rounded bg-transparent hover:bg-nova-elements-background-depth-3"
                  onClick={() => {
                    if (item.type === 'file') {
      ...
      {/* Footer */}
      <div className="px-2 py-1 border-t border-nova-elements-borderColor bg-nova-elements-background-depth-2 text-xs text-nova-elements-textTertiary flex justify-between items-center">
        <div>

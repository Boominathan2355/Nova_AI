...
const FullscreenButton = memo(({ onClick, isFullscreen }: FullscreenButtonProps) => (
  <button
    onClick={onClick}
    className="ml-4 p-1 rounded hover:bg-nova-elements-background-depth-3 text-nova-elements-textTertiary hover:text-nova-elements-textPrimary transition-colors"
...
const FullscreenOverlay = memo(({ isFullscreen, children }: { isFullscreen: boolean; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-6">
      <div className="w-full h-full max-w-[90vw] max-h-[90vh] bg-nova-elements-background-depth-2 rounded-lg border border-nova-elements-borderColor shadow-xl overflow-hidden">
        {children}
      </div>
    </div>
  );
...
const lineNumberStyles =
  'w-9 shrink-0 pl-2 py-1 text-left font-mono text-nova-elements-textTertiary border-r border-nova-elements-borderColor bg-nova-elements-background-depth-1';
const lineContentStyles =
  'px-1 py-1 font-mono whitespace-pre flex-1 group-hover:bg-nova-elements-background-depth-2 text-nova-elements-textPrimary';
...
const changeColorStyles = {
  ...
  unchanged: 'text-nova-elements-textPrimary',
};

const renderContentWarning = (type: 'binary' | 'error') => (
  <div className="h-full flex items-center justify-center p-4">
    <div className="text-center text-nova-elements-textTertiary">
      <div className={`i-ph:${type === 'binary' ? 'file-x' : 'warning-circle'} text-4xl text-red-400 mb-2 mx-auto`} />
      <p className="font-medium text-nova-elements-textPrimary">
  ...
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-nova-elements-textTertiary">
        ...
        <p className="font-medium text-nova-elements-textPrimary">Files are identical</p>
        <p className="text-sm mt-1">Both versions match exactly</p>
      </div>
      <div className="mt-4 w-full max-w-2xl bg-nova-elements-background-depth-1 rounded-lg border border-nova-elements-borderColor overflow-hidden">
        <div className="p-2 text-xs font-bold text-nova-elements-textTertiary border-b border-nova-elements-borderColor">
          Current Content
        </div>
    ...
    return (
      <div className="flex group min-w-fit">
        ...
        <div className={`${lineContentStyles} ${bgColor}`}>
          <span className="mr-2 text-nova-elements-textTertiary">
...
const FileInfo = memo(
  ({
    ...
    return (
      <div className="flex items-center bg-nova-elements-background-depth-1 p-2 text-sm text-nova-elements-textPrimary shrink-0">
        ...
        <span className="ml-auto shrink-0 flex items-center gap-2">
          {hasChanges ? (
            <>
              ...
              <span className="text-nova-elements-textTertiary text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
  ...
  if (!highlighter) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-nova-elements-textTertiary">Loading diff...</div>
      </div>
    );
  }
...
export const DiffView = memo(({ fileHistory, setFileHistory }: DiffViewProps) => {
  ...
  try {
    ...
    return (
      <div className="flex w-full h-full justify-center items-center bg-nova-elements-background-depth-1 text-red-400">

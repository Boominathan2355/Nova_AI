...
export function Search() {
  ...
  return (
    <div className="flex flex-col h-full bg-nova-elements-background-depth-2">
      {/* Search Bar */}
      <div className="flex items-center py-3 px-3">
        <div className="relative flex-1">
          <input
            ...
            placeholder="Search"
            className="w-full px-2 py-1 rounded-md bg-nova-elements-background-depth-3 text-nova-elements-textPrimary placeholder-nova-elements-textTertiary focus:outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto py-2">
        {isSearching && (
          <div className="flex items-center justify-center h-32 text-nova-elements-textTertiary">
        ...
        {!isSearching &&
          Object.keys(groupedResults).map((file) => (
            <div key={file} className="mb-2">
              <button
                className="flex gap-2 items-center w-full text-left py-1 px-2 text-nova-elements-textSecondary bg-transparent hover:bg-nova-elements-background-depth-3 group"
                onClick={() => setExpandedFiles((prev) => ({ ...prev, [file]: !prev[file] }))}
              >
                <span
                  className=" i-ph:caret-down-thin w-3 h-3 text-nova-elements-textSecondary transition-transform"
                  style={{ transform: expandedFiles[file] ? 'rotate(180deg)' : undefined }}
                />
                <span className="font-normal text-sm">{file.split('/').pop()}</span>
                <span className="h-5.5 w-5.5 flex items-center justify-center text-xs ml-auto bg-nova-elements-item-backgroundAccent text-nova-elements-item-contentAccent rounded-full">
                ...
                <div className="">
                  {groupedResults[file].map((match, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="hover:bg-nova-elements-background-depth-3 cursor-pointer transition-colors pl-6 py-1"
                        onClick={() => handleResultClick(match.path, match.lineNumber)}
                      >
                        <pre className="font-mono text-xs text-nova-elements-textTertiary truncate">
                          {!isStart && <span>...</span>}
                          {previewText.slice(0, matchStart)}
                          <span className="bg-nova-elements-item-backgroundAccent text-nova-elements-item-contentAccent rounded px-1">

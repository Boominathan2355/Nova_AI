...
export function SupabaseConnection() {
  ...
  return (
    <div className="relative">
      <div className="flex border border-nova-elements-borderColor rounded-md overflow-hidden mr-2 text-sm">
        <Button
          active
          disabled={connecting}
          onClick={() => setIsDialogOpen(!isDialogOpen)}
          className="hover:bg-nova-elements-item-backgroundActive !text-white flex items-center gap-2"
        >
          <img
            className="w-4 h-4"
            height="20"
            ...
            {!isConnected ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg">
                  <div>
                    <h4 className="text-sm font-medium text-nova-elements-textPrimary">{supabaseConn.user?.email}</h4>
                    <p className="text-xs text-nova-elements-textSecondary">Role: {supabaseConn.user?.role}</p>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <button
                        ...
                        className="bg-transparent text-left text-sm font-medium text-nova-elements-textPrimary flex items-center gap-2"
                      >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-2 py-1 rounded-md text-xs bg-[#F0F0F0] dark:bg-[#252525] text-nova-elements-textSecondary hover:bg-[#E5E5E5] dark:hover:bg-[#333333] flex items-center gap-1"
                    ...
                    {isProjectsExpanded && (
                      <>
                        {!supabaseConn.selectedProjectId && (
                          <div className="mb-2 p-3 bg-[#F8F8F8] dark:bg-[#1A1A1A] rounded-lg text-sm text-nova-elements-textSecondary">
                            ...
                            {supabaseConn.stats.projects.map((project) => (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <div>
                                    <h5 className="text-sm font-medium text-nova-elements-textPrimary flex items-center gap-1">
                                      <div className="i-ph:database w-3 h-3 text-[#3ECF8E]" />
                                      {project.name}
                                    </h5>
                                    <div className="text-xs text-nova-elements-textSecondary mt-1">
                                      {project.region}
                                    </div>
                                  </div>
                                  <button
                                    ...
                                    className={classNames(
                                      ...
                                      supabaseConn.selectedProjectId === project.id
                                        ? 'bg-[#3ECF8E] text-white'
                                        : 'bg-[#F0F0F0] dark:bg-[#252525] text-nova-elements-textSecondary hover:bg-[#3ECF8E] hover:text-white',
                                    )}
                                  >
                                    {supabaseConn.selectedProjectId === project.id ? (
  ...
  return (
    <button
      className={classNames(
        ...
        {
          'bg-nova-elements-item-backgroundDefault hover:bg-nova-elements-item-backgroundActive text-nova-elements-textTertiary hover:text-nova-elements-textPrimary':
            !active,
          'bg-nova-elements-item-backgroundDefault text-nova-elements-item-contentAccent': active && !disabled,
          'bg-nova-elements-item-backgroundDefault text-alpha-gray-20 dark:text-alpha-white-20 cursor-not-allowed':
            disabled,
        },
        className,
      )}

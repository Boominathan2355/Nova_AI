      ...
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__boltSelectedElement__')) {
            ...
            return (
              <div className="bg-nova-elements-background-depth-3 border border-nova-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-nova-elements-background-depth-2 px-2 py-1 rounded text-nova-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-nova-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-nova-elements-textSecondary !bg-nova-elements-background-depth-2 border border-nova-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__novaThought__')) {
            return <ThoughtBox title="Thought process">{children}</ThoughtBox>;
          }

          if (className?.includes('__boltQuickAction__') || dataProps?.dataNovaQuickAction) {
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            ...
            dataProps?.dataNovaQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-nova-elements-item-backgroundAccent text-nova-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}

    ...
    return (
      <div className={classNames('flex flex-col items-center justify-center py-8', className)}>
        <div className="relative mb-4">
          <Loader2 className="w-8 h-8 animate-spin text-nova-elements-item-contentAccent" />
          {showProgress && progress > 0 && (
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-xs font-medium text-nova-elements-item-contentAccent">{progress}%</span>
            </div>
          )}
        </div>

        <div className="text-center space-y-2">
          <p className="text-sm font-medium text-nova-elements-textPrimary">{loadingMessage}</p>

          {showProgress && progressSteps.length > 0 && (
            <div className="w-full max-w-sm">
              {/* Progress bar */}
              <div className="w-full bg-nova-elements-background-depth-2 rounded-full h-2 mb-3">
                <motion.div
                  className="bg-nova-elements-item-contentAccent h-2 rounded-full"
                  initial={{ width: 0 }}
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 text-xs text-nova-elements-textSecondary hover:text-nova-elements-textPrimary transition-colors"
              >
                    ...
                    {progressSteps.map((step) => (
                      <div key={step.key} className="flex items-center gap-2 text-xs">
                        {step.error ? (
                          ...
                          <Loader2 className="w-3 h-3 animate-spin text-nova-elements-item-contentAccent flex-shrink-0" />
                        ) : (
                          <div className="w-3 h-3 rounded-full border border-nova-elements-borderColor flex-shrink-0" />
                        )}
                        <span
                          className={classNames(
                            step.error
                              ...
                              : step.completed
                                ...
                                : step.loading
                                  ...
                                  : 'text-nova-elements-textSecondary',
                          )}
                        >
                          {step.label}
                        </span>
  ...
  if (error) {
    return (
      <div className={classNames('flex flex-col items-center justify-center py-8 text-center space-y-4', className)}>
        ...
        <div>
          <h3 className="text-sm font-medium text-nova-elements-textPrimary mb-1">Failed to Load</h3>
          <p className="text-xs text-nova-elements-textSecondary mb-4 max-w-sm">{error}</p>
        </div>
    ...
    <div className={classNames('relative', className)}>
      {isRefreshing && (
        <div className="absolute top-0 right-0 z-10">
          <div className="flex items-center gap-2 px-2 py-1 bg-nova-elements-background-depth-1 border border-nova-elements-borderColor rounded-lg shadow-sm">
            <Loader2 className="w-3 h-3 animate-spin text-nova-elements-item-contentAccent" />
            <span className="text-xs text-nova-elements-textSecondary">{refreshingMessage}</span>
          </div>
        </div>
      )}

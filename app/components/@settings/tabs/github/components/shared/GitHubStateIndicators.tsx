  ...
  return (
    <div
      className={classNames(
        'flex flex-col items-center justify-center py-8 text-nova-elements-textSecondary',
        className,
      )}
    >
      <Loader2 className={classNames('animate-spin mb-2', sizeClasses[size])} />
      <p className={classNames('text-nova-elements-textSecondary', textSizeClasses[size])}>{message}</p>
    </div>
  );
}
...
export function ErrorState({
  ...
  return (
    <div className={classNames('flex flex-col items-center justify-center py-8 text-center', className)}>
      ...
      <h3 className={classNames('font-medium text-nova-elements-textPrimary mb-1', textSizeClasses[size])}>{title}</h3>
      <p className={classNames('text-nova-elements-textSecondary mb-4', textSizeClasses[size])}>{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className="px-4 py-2 bg-nova-elements-item-contentAccent text-white rounded-lg hover:bg-nova-elements-item-contentAccent/90 transition-colors"
        >
          {retryLabel}
        </button>
      )}
...
export function GitHubConnectionRequired({ onConnect, className = '' }: GitHubConnectionRequiredProps) {
  return (
    <div className={classNames('flex flex-col items-center justify-center py-12 text-center', className)}>
      <Github className="w-12 h-12 text-nova-elements-textTertiary mb-4" />
      <h3 className="text-lg font-medium text-nova-elements-textPrimary mb-2">GitHub Connection Required</h3>
      <p className="text-sm text-nova-elements-textSecondary mb-6 max-w-md">
      ...
      {onConnect && (
        <button
          onClick={onConnect}
          className="px-6 py-3 bg-nova-elements-item-contentAccent text-white rounded-lg hover:bg-nova-elements-item-contentAccent/90 transition-colors flex items-center gap-2"
        >

...
export function RepositoryCard({
    ...
    return (
      <button
        ...
        className={classNames(
          'w-full text-left p-3 rounded-lg border border-nova-elements-borderColor hover:border-nova-elements-borderColorActive hover:bg-nova-elements-background-depth-1 transition-all duration-200',
          className,
        )}
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium text-nova-elements-textPrimary">{repository.name}</h4>
            {repository.private && <Lock className="w-3 h-3 text-nova-elements-textTertiary" />}
            {repository.fork && <GitFork className="w-3 h-3 text-nova-elements-textTertiary" />}
            {repository.archived && <Archive className="w-3 h-3 text-nova-elements-textTertiary" />}
          </div>

          <div className="flex items-center gap-3 text-xs text-nova-elements-textSecondary">
        ...
        {repository.description && (
          <p className="text-xs text-nova-elements-textSecondary mb-2 line-clamp-2">{repository.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-nova-elements-textTertiary">
            {repository.language && (
              <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1 text-xs text-nova-elements-textTertiary">
  ...
  const interactiveProps = onSelect
    ? {
        onClick: onSelect,
        className: classNames(
          'group cursor-pointer hover:border-nova-elements-borderColorActive dark:hover:border-nova-elements-borderColorActive transition-all duration-200',
          className,
        ),
      }
    : { className };
  ...
  return (
    <Component
      {...interactiveProps}
      className={classNames(
        'block p-4 rounded-lg bg-nova-elements-background-depth-1 dark:bg-nova-elements-background-depth-1 border border-nova-elements-borderColor dark:border-nova-elements-borderColor relative',
    ...
    >
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-nova-elements-icon-tertiary" />
            <h5
              className={classNames(
                'text-sm font-medium text-nova-elements-textPrimary',
                onSelect && 'group-hover:text-nova-elements-item-contentAccent transition-colors',
              )}
            >
              {repository.name}
            </h5>
            {repository.fork && (
              <span title="Forked repository">
                <GitFork className="w-3 h-3 text-nova-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title="Archived repository">
                <Archive className="w-3 h-3 text-nova-elements-textTertiary" />
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-nova-elements-textSecondary">
            <span className="flex items-center gap-1" title="Stars">
              <Star className="w-3.5 h-3.5 text-nova-elements-icon-warning" />
              {repository.stargazers_count.toLocaleString()}
            </span>
            <span className="flex items-center gap-1" title="Forks">
              <GitFork className="w-3.5 h-3.5 text-nova-elements-icon-info" />
              {repository.forks_count.toLocaleString()}
            </span>
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title="Open Issues">
                <Circle className="w-3.5 h-3.5 text-nova-elements-icon-error" />
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title="Pull Requests">
                <GitPullRequest className="w-3.5 h-3.5 text-nova-elements-icon-success" />
        ...
        <div className="space-y-2">
          {repository.description && (
            <p className="text-xs text-nova-elements-textSecondary line-clamp-2">{repository.description}</p>
          )}

          {/* Repository metrics bar */}
          <div className="flex items-center gap-2 text-xs">
            {repository.license && (
              <span className="px-2 py-0.5 rounded-full bg-nova-elements-background-depth-2 text-nova-elements-textTertiary">
            ...
            {onSelect && (
              <span
                className={classNames(
                  ...
                  'group-hover:text-nova-elements-item-contentAccent',
                )}
              >

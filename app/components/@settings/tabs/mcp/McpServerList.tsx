...
export default function McpServerList({
  ...
  if (serverEntries.length === 0) {
    return <p className="text-sm text-nova-elements-textSecondary">No MCP servers configured</p>;
  }
  ...
  return (
    <div className="space-y-2">
      {filteredEntries.map(([serverName, mcpServer]) => {
        ...
        return (
          <div key={serverName} className="flex flex-col p-2 rounded-md bg-nova-elements-background-depth-1">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <div
                  onClick={() => toggleServerExpanded(serverName)}
                  className="flex items-center gap-1.5 text-nova-elements-textPrimary"
                ...
                <div className="flex-1 min-w-0 truncate">
                  {mcpServer.config.type === 'sse' || mcpServer.config.type === 'streamable-http' ? (
                    <span className="text-xs text-nova-elements-textSecondary truncate">{mcpServer.config.url}</span>
                  ) : (
                    <span className="text-xs text-nova-elements-textSecondary truncate">
            ...
            {isExpanded && isAvailable && (
              <div className="mt-2">
                <div className="text-nova-elements-textSecondary text-xs font-medium ml-1 mb-1.5">Available Tools:</div>
                {serverTools.length === 0 ? (
                  <div className="ml-4 text-xs text-nova-elements-textSecondary">No tools available</div>
                ) : (

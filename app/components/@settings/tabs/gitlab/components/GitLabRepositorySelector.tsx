...
export function GitLabRepositorySelector({ onClone, className }: GitLabRepositorySelectorProps) {
  ...
  if (!isConnected || !connection) {
    return (
      <div className="text-center p-8">
        <p className="text-nova-elements-textSecondary mb-4">Please connect to GitLab first to browse repositories</p>
    ...
    return (
      <div className="text-center p-8">
        <div className="text-red-500 mb-4">
          ...
          <p className="text-sm text-nova-elements-textSecondary mt-1">{error}</p>
        </div>
  ...
  if (isLoading && !repositories.length) {
    return (
      <div className="flex flex-col items-center justify-center p-8 space-y-4">
        <div className="animate-spin w-8 h-8 border-2 border-nova-elements-borderColorActive border-t-transparent rounded-full" />
        <p className="text-sm text-nova-elements-textSecondary">Loading repositories...</p>
      </div>
    );
  }
  ...
  if (!repositories.length && !isLoading) {
    return (
      <div className="text-center p-8">
        <GitBranch className="w-12 h-12 text-nova-elements-textTertiary mx-auto mb-4" />
        <p className="text-nova-elements-textSecondary mb-4">No repositories found</p>
        <Button variant="outline" onClick={handleRefresh} disabled={isRefreshing}>
          <RefreshCw className={classNames('w-4 h-4 mr-2', { 'animate-spin': isRefreshing })} />
          Refresh
        </Button>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-nova-elements-textPrimary">Select Repository to Clone</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-nova-elements-textTertiary" />
          <input
            type="text"
            placeholder="Search repositories..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 rounded-lg bg-nova-elements-background-depth-1 border border-nova-elements-borderColor text-nova-elements-textPrimary placeholder-nova-elements-textTertiary focus:outline-none focus:ring-1 focus:ring-nova-elements-borderColorActive"
          />
        </div>

        {/* Sort */}
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-nova-elements-textTertiary" />
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortOption)}
            className="px-3 py-2 rounded-lg bg-nova-elements-background-depth-1 border border-nova-elements-borderColor text-nova-elements-textPrimary text-sm focus:outline-none focus:ring-1 focus:ring-nova-elements-borderColorActive"
          >
        ...
        {/* Filter */}
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-nova-elements-textTertiary" />
          <select
            value={filterBy}
      ...
      {currentRepositories.length > 0 ? (
        <>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t border-nova-elements-borderColor">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-nova-elements-textSecondary px-3">
        ...
        <div className="text-center py-8">
          <p className="text-nova-elements-textSecondary">No repositories found matching your search criteria.</p>
        </div>
      )}
